...
- **Traefik's timeouts also answer to the fleet-wide names.**
  ...
  in the fleet. When set they win; `DIFY_REQUEST_TIMEOUT` keep working exactly as before,
...
- **CI had never run the restore scripts.** The test restored with its own
  ...
  stopped. The data restore cleared with `rm -rf dir/*`, which leaves every
...
- **`busybox:1.37` was rebuilt upstream**; the pin moved from `sha256:9db7b59979c3…` to `sha256:bdf57e528e45…`. Same version, same tag, a rebuilt base image — the usual shape of a security fix in a base layer.
...
- **`redis:7.4-alpine` was rebuilt upstream**; the pin moved from `sha256:520775a41a63…` to `sha256:858f009f9709…`. Same version, same tag, a rebuilt base image — the usual shape of a security fix in a base layer.
- **`postgres:15-alpine` was rebuilt upstream**; the pin moved from `sha256:a46e076249ce…` to `sha256:f7d23353e1b1…`. Same version, same tag, a rebuilt base image — the usual shape of a security fix in a base layer.
...
[Unreleased]: https://github.com/heyvaldemar/dify-traefik-letsencrypt-docker-compose/compare/v1.1.0...HEAD
[1.1.0]: https://github.com/heyvaldemar/dify-traefik-letsencrypt-docker-compose/compare/v1.0.5...v1.1.0
[1.0.4]: https://github.com/heyvaldemar/dify-traefik-letsencrypt-docker-compose/compare/v1.0.3...v1.0.4
