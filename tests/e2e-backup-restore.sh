#!/bin/bash
# End-to-end tests for the dify-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-dify}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-dify-traefik-letsencrypt-docker-compose.yml}"

if [[ -f .env ]]; then
  # Read, do not execute. A compose .env file is key=value with literal
  # values, not a shell script: Dify's own LOG_DATEFORMAT is
  # `%Y-%m-%d %H:%M:%S`, and `source` reads the second half of that as a job
  # spec and dies on "fg: no job control". Assigning each line by hand keeps
  # every value exactly as compose will see it.
...
: "${POSTGRES_BACKUPS_PATH:=/srv/dify-postgres/backups}"
: "${POSTGRES_BACKUP_NAME:=dify-postgres-backup}"
: "${BACKUP_INTERVAL:=24h}"
: "${DIFY_DB_NAME:=dify}"
: "${DIFY_DB_USER:=postgres}"
: "${DATA_BACKUPS_PATH:=/srv/dify-application-data/backups}"
: "${DATA_BACKUP_NAME:=dify-application-data-backup}"

BACKUPS_PATH="${POSTGRES_BACKUPS_PATH%/}"
BACKUP_PREFIX="${POSTGRES_BACKUP_NAME}"
BACKUP_EXT=".gz"
INTERVAL="${BACKUP_INTERVAL}"
DB_NAME="${DIFY_DB_NAME}"
DB_USER="${DIFY_DB_USER}"
DB_HOST="db_postgres"
...
DB_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq db_postgres | head -n 1)"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./dify-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./dify-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./dify-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./dify-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
